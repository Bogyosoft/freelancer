...
export class TableTemplateComponent implements OnInit, OnChanges {
  ...
  createCSVfromTableData(){
    let json = Object.values(this.tableData.filteredData);
    console.log(this.tableData.filteredData);
    var csv = "";
    var keys = ((json[0] && Object.keys(json[0])) || []).filter((element: string) => this.displayedColumns.includes(element));
    csv += keys.join(',') + '\n';
    console.log(csv);
    ...
    console.log(csv);
    this.createDownloadableFromString(csv);
  }

  createDownloadableFromString(input:string){
